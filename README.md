...
Practical PowerShell scripts for Entra ID identity governance — directory extensions, guest access lifecycle, dynamic license groups, and managed identity permissions.

Built from real-world consulting work. These scripts solve problems that documentation glosses over.

Related blog posts at agderinthe.cloud
...
Logic Apps/Distribution List Membership	Automated management of Exchange Online distribution list memberships via Entra ID Entitlement Management using a no-secrets Bicep architecture.
Prerequisites
PowerShell 7+
...
Required permissions: Application.Read.All, AppRoleAssignment.ReadWrite.All, RoleManagement.ReadWrite.Directory
...
Good identity governance is less about clever PowerShell and more about asking the right questions — of your HR system, your business stakeholders, and your own assumptions. These scripts are designed to support well-designed processes, not replace them.
...
Directory extensions over Exchange attributes — for governance metadata that needs to work across systems.
Contributing
